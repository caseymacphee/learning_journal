...
TEST_DSN = 'dbname=test_learning_journal user=postgres password=becreative'
...
def test_app():
	#configure the app for use in test_learning_journal
...
def with_entry(db, request):
	...
	with app.test_request_context('/'):
		write_entry(*expected)
		#manually commit transactin here to avoid rollback due to handled exceptions
